...
## 项目简介
TokenSavingPlugin 是一款面向 SillyTavern 的**上下文压缩插件**。
依托自研 **n+m 分段生长策略**，当对话长度超出阈值时，插件会自动将久远的对话内容压缩为摘要，并追加至 **FCC（冻结压缩典籍）** 块。在完整保留对话叙事连贯性的前提下，大幅降低 Token 消耗。
...
| 02:00~03:00 | 524,288 | 56,079 | 13,719 | 580,367 | 90.34% |
| 2026-09-11（全天） | 7,282,680 | 387,842 | 161,945 | 7,670,522 | 94.94% |
...
<details>
<summary><b>点击展开：缓存命中原理</b></summary>1. FCC 采用 Append-Only（仅追加）模式，**绝不修改已经生成的摘要内容**
2. FCC 在 Prompt 中位置固定，不会随对话轮次滑动移位
3. `系统提示词 + 角色卡 + 世界书 + FCC` 共同构成稳定不变的前缀
4. DeepSeek / Anthropic / OpenAI 等服务商的 Prompt Cache 机制会持续命中这部分固定前缀，从而享受缓存计费折扣
