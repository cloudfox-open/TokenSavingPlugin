...
### 项目简介
TokenSavingPlugin 是一款面向 SillyTavern 的**上下文压缩和提高缓存命中的插件，可以将原本50%缓存的命中，提高到90%甚至94%**。
依托**n+m 分段生长策略**，当对话长度超出阈值时，插件会自动将久远的对话内容压缩为摘要，并追加至 **FCC（冻结压缩典籍）** 块。在完整保留对话叙事连贯性的前提下，大幅降低 Token 消耗。


**如果你厌倦了每天用sillytavern聊上百句话，回头一看账单，因为缓存命中过低，只有45%~55%，导致每月api开销就是30元或更多，那么此插件非常适合你，能将你的45%提到至少90%的缓存命中。让你不再因为想聊，却还欠梁圣0.08元，导致被大肥鱼直接拒绝，看着屏幕上的一行字：余额不足，请登录平台查看余额信息。从而心情失落？此插件就是为了解决这个问题从摇篮中诞生。**


**实际使用情况如下**
你打开sillytavern，安装TokenSavingPlugin，看着雪白的面板，在酒馆的响应配置里，设置上下文长度为8192或更高，然后点击插件面板的“自动”，就会为您自动填入对应的参数，无需思考，简单快捷。然后尝试进行上百次或上千次的对话，你会发现，账单从原本每月固定30元开销，变成了10元甚至更低，既解决了酒馆痛点，还充实你的钱包。
...
### 当你在酒馆里调用api愉快聊天的时候，是不是咔嚓一下发现账单爆炸？是不是手忙脚乱切换时区想要拯救一下，结果发现几百万的提示词全部未命中？
就像图中这样
...
3. 判断触发条件：窗口溢出 (轮数 > n+m)  OR  Token ≥ 阈值
4. 锁定输入框 + 隐藏待压缩消息
5. 分块提交至AI进行压缩（单块上限 ≤ 800 token）
6. Feynman 自检（可选）
7. 追加摘要到FCC + 持久化保存 + 注入Prompt
8. 解锁输入框

### 常见问题

<details>
<summary><b>Q1: 为什么插件刚开始聊天不会触发压缩？</b></summary>压缩需要满足**任一**条件才会启动：可见对话轮数超过 `n + m`；或者 Token 占比达到阈值（默认 80%）。对话前几轮属于 “生长阶段”，正常继续聊天即可。

</details><details>
<summary><b>Q2: 压缩触发太频繁 / 很久都不压缩？</b></summary><ul>
<li><b>压缩过于频繁</b>：增大参数 <code>m</code>，或直接切换「省成本」预设</li>
<li><b>压缩很久不触发</b>：减小参数 <code>m</code>，或切换「省内存」预设</li>
</ul></details><details>
<summary><b>Q3: 压缩会不会丢失剧情记忆？</b></summary><b>不会</b>，三层安全保护：
...
<summary><b>Q4: 支持哪些 API 服务商？</b></summary>支持所有 SillyTavern 可对接的 API：DeepSeek / OpenAI / Claude / Gemini / 本地大模型等。
...
<div align="center">如果这个项目对你有帮助，欢迎点亮 Star 

Made with by cloudfox-open
